...
const SocialBtn = ({ platform, small, ticker, title }) => {
  ...
  const handleShare = (e) => {
    ...
    let url = "";
    if (platform === 'Twitter') url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out this autonomous neural synthesis on ${ticker || 'the markets'}: ${title ? title + '.' : 'Institutional-grade research generated by Ecotron.'}\n\n#Finance #Trading #Quant #AI`)}&url=${encodeURIComponent(shareUrl)}`;
    if (platform === 'Facebook') url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`;
    if (platform === 'WhatsApp') url = `https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out this AI-generated financial research on ${ticker || 'the markets'}: ${title || ''}\n\n${shareUrl}`)}`;
